...
const FunnelPage = async ({ params }: Props) => {
  const funnelPages = await getFunnel((await params).funnelId);
  if (!funnelPages)
    return redirect(`/subaccount/${(await params).subaccountId}/funnels`);

  return (
    <BlurPage>
      <Link
        href={`/subaccount/${(await params).subaccountId}/funnels`}
